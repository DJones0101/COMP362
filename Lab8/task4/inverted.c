...
int findOldIndex(struct inverted_ptable *inverted_table, int pid, int page) {
	...
	time_t max_time = inverted_table->table[0]->time_stamp;
	double compare_times;

	for (index = 0; index < number_of_pages; index++) {

		
		compare_times = difftime(current_time_stamp, max_time);
...
int findEmpty(struct inverted_ptable *inverted_table, int pid, int page) {
	...
	for (index = 0; index < number_of_pages; index++) {
		...
		if (inverted_table->table[index]->pid == 0 && inverted_table->table[index]->page == 0) {
